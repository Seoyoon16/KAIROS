import time, serial, serial.tools.list_ports, threading
...
if __name__ == '__main__':
    ...
    ports = list(serial.tools.list_ports.comports()) # 통신포트 연결
    for p in ports:
        if 'CH340' in p.description:
            print(f"{p} 포트에 연결하였습니다.")
            my_serial = serial.Serial(p.device, baudrate=9600, timeout=1.0)
    
    t1 = threading.Thread(target=serial_read_thread) # 병렬처리
    t1.daemon = True
    t1.start()
